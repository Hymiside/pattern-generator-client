...
async def start(message: types.Message):
    ...
    match response["status"]:
        case "user already exists":
            await message.answer("*Cоздавай* новые интересные *паттерны* из сообщений в Telegram 👇",
                                 reply_markup=keyboard.main_keyboard(), parse_mode=types.ParseMode.MARKDOWN_V2)
...
async def get_balance(callback: types.CallbackQuery):
    ...
    match response["status"]:
        case "ok":
            await callback.message.edit_text(f"Ваш баланс: *{response['balance']} рублей*", parse_mode=types.ParseMode.MARKDOWN_V2)
...
@dp.callback_query_handler(lambda c: c.data and c.data.startswith("rub"))
async def choose_numbers_cash(callback: types.CallbackQuery):
    numbers_cash: str = callback.data[3:]
    # TODO


@dp.callback_query_handler(text="back")
async def update_cash(callback: types.CallbackQuery):
    await callback.message.edit_text("*Cоздавай* новые и интересные *паттерны из сообщений* в Telegram 👇", parse_mode=types.ParseMode.MARKDOWN_V2)
